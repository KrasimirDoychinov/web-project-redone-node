...
describe('postServices', () => {
    ...
    describe('create', () => {
        ...
        beforeEach(() => {
            dbMock = [];
            mockObj = {description: 'description', thread: {}, creator: {}};
        });

        before(() => {
            const saveStub = stub(Post.prototype, 'save').callsFake(() => {
                dbMock.push(mockObj);     
            });
        ...
        it('should increase count in dbMock', () => {
            return postServices.create(mockObj.description, mockObj.thread, mockObj.creator).then(res => {
        ...
        const errorObjs = [
            ...
            {description: undefined, thread: {title: 'test'}, creator: {name: 'test'}},
            {description: 'test', thread: {title: 'test'}, creator: undefined},
        ]
        errorObjs.forEach(x => {
            it('should return obj with error msg if one property is null', () => {
                return postServices.create(x.description, x.thread, x.creator).then(res => {
    ...
    describe('getById', () => {
        ...
        beforeEach(() => {
            dbMock = [];
            mockObj = {description: 'description', thread: {}, creator: {}, id: 1};
        });
        
        before(() => {
            const postServiceStub = stub(postServices, 'create').callsFake(() => {
                dbMock.push(mockObj);
            });  	

            const findByIdStub = stub(Post, 'findById').callsFake(() => {
    ...
    describe('deleteById', () => {
        ...
        before(() => {
            const findByIdAndDeleteStub = stub(Post, 'findByIdAndDelete').callsFake(() => {
                return dbMock.filter(x => x.id != 1);
            });
        });
